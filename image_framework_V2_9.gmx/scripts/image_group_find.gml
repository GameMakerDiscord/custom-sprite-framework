...
var _min_area_id   = -1;
/*var _w = argument0[ __IS_FINISH.IMG_WIDTH ];
var _h = argument0[ __IS_FINISH.IMG_HEIGHT ];*/
...
for( var n = 0; n < ds_list_size( _l_areas ); n += 4 ){
    if( _w <= _l_areas[| n + 2 ] and _h <= _l_areas[| n + 3 ] ){
        var _mi = min(_l_areas[| n + 2 ], _l_areas[| n + 3 ]);
        var _ma = min(_l_areas[| n + 2 ], _l_areas[| n + 3 ]);
        var _va = _mi;
        
        if( _va < _min_area_size ){
            _min_area_size = _va;
            _min_area_id = n;
        }
    }
...
_g_image[# _pos + __ISG_SUBIMG.BACK, 0 ] = _l_texpage[| ds_list_size( _l_texpage ) - 1 ]; // BACKGROUND
_g_image[# _pos + __ISG_SUBIMG.X, 0 ]    = _l_areas[| _area_id ];     // X
_g_image[# _pos + __ISG_SUBIMG.Y, 0 ]    = _l_areas[| _area_id + 1 ]; // Y

_pos += __ISG_SUBIMG.COUNT;
/*_g_image[# _pos, 0 ] = _subimg_w; _pos++;     // W
_g_image[# _pos, 0 ] = _subimg_h; _pos++; // H*/
...
if( true || _l_areas[| _area_id + 2 ] <= _l_areas[| _area_id + 3 ] ){
    /*
     if width > height, the primary/larger area is vertical to the subimage
    */
    ...
    /*
     if height > width, the primary/larger area is horisontal to the subimage
    */
    // AREA = X, Y, W, H
    if( _subimg_w < _l_areas[| _area_id + 2 ] ){
        ds_list_add( _l_areas, 
